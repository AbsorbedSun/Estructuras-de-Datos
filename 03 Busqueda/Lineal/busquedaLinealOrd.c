/*  
busquedaLinealOrd.c (Busqueda Lineal)
Autor: Aldo Garcia Ambrosio (C) Abril 2025
Autor origina: Edgardo Adrián Franco Martínez (pseudocódigo)
Uso de IA (GPT o4, Claude)
Version: 1.0
  
Programa que recibe aun arreglo ordenado y un elemento a buscar y realiza una busqueda lineal a un arreglo de n elementos.
  
Complejidad: O(n).
  
Observaciones: 

Compilación: Windows / Linux: gcc busquedaLinealOrd.c -o bLinealOrd

Ejecución: ./bLinealOrd {elementos del arreglo} {elemento a buscar} < lista_caracteres.txt
            ./bLinealOrd {elementos del arreglo} {elemento a buscar} < lista_caracteres.txt > salida.txt
*/
#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>
...
int busquedaLinealOrd(int *ARREGLO, int n, int elemento, int *comparaciones);
...
para hallar un elemento dentro de un ARREGLO de n elementos ordenados, ademas de imprimir el numero de comparaciones realizadas.
*/
int main(int numArg, char *argUser[]){
    ...
    int posicion = busquedaLinealOrd(ARREGLO, n, elemento, &comparaciones);
    if(posicion != -1) {
...
/* 
int busquedaLinealOrd(int *ARREGLO, int n, int elemento)
Recibe: int *ARREGLO como puntero al arreglo ordenado, n como tamaño del arreglo y elemento como valor a buscar
Devuelve: int que representa la posición del elemento buscado o -1 si no se encuentra
Observaciones: Función que implementa el algoritmo de búsqueda lineal optimizado para arreglos ordenados,
recorriendo el arreglo elemento por elemento hasta encontrar el valor buscado o hasta encontrar un elemento
mayor que el buscado (lo que indica que el elemento no está en el arreglo).
*/
int busquedaLinealOrd(int *ARREGLO, int n, int elemento, int *comparaciones) {
   ...
   // Recorre el arreglo hasta el ultimo elemento del arreglo o hasta encontrar un elemento mayor que el buscado
   for(int i = 0; i <= (n-1) || ARREGLO[i] > elemento; i++) {
       ...
       if (ARREGLO[i] == elemento) {
           resultado = i;
           break;  // Sale del ciclo al encontrar la coincidencia
       }
   }
