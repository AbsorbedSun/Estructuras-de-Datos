...
void Flip(cola *c) {
    ...
    // Desencolar todos los elementos de la cola original
    // y encolarlos en orden inverso en otra estructura (puede ser un arreglo o pila)
    elemento elementos[100]; // Asumiendo un tamaño máximo, ajusta según necesites
    int tam = 0;
    
    while(!Empty(c)) {
        elementos[tam++] = Dequeue(c);
    }
    
    // Encolar los elementos en orden inverso de vuelta a la cola original
