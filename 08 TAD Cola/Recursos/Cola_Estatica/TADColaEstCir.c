...
Descripción: Busca un elemento en la cola estática circular y retorna su posición
...
int Search(cola *c, elemento e){
    ...
    // Si la cola está vacía, no hay que buscar
    if(Empty(c))
        return 0;
    
    // Recorremos la cola buscando el elemento
    for(i = 1; i <= c->num_elem; i++){
        elemento temp = Element(c, i);
        if(memcmp(&temp, &e, sizeof(elemento)) == 0)
            return i;
    }
...
/*
void Flip(cola *c)
Descripción: Invierte el orden de los elementos en la cola estática circular
Recibe: cola *c (Referencia a la cola "c" a operar)
Devuelve: Nada
...
void Flip(cola *c){
    ...
    // Extraemos todos los elementos en orden y los guardamos en un arreglo temporal
    for(i = 0; i < tam; i++){
        elementos[i] = Dequeue(c);
    }
    
    // Ahora los volvemos a insertar en orden inverso
    for(i = tam - 1; i >= 0; i--){
        Queue(c, elementos[i]);
    }
}